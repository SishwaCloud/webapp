...
async function  createNewProduct(params, req, res) {
    ...
    if (await mySqlDb.Product.findOne({ where: { sku: params.sku } })) {
...
const product = await mySqlDb.Product.create(params);
...
async function updateProductValues(req,res){
    ...
    if (await mySqlDb.Product.findOne({ where: { sku: updateProduct.sku } })) {
...
async function deleteProduct(productId, req) {
    const product = await mySqlDb.Product.findByPk(productId);
    ...
    if(userId != product.dataValues.owner_user_id){
        ...
        mySqlDb.Product.destroy({ where: { id: productId } })
}
    return product;
}
